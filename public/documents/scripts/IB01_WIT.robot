...
${Retry_IB01_WIT_PIX}   45x
${Retry_Interval_IB01_WIT_PIX}    30 seconds
...
IB01_WIT_TC01_Inbound_Happy_Path_SAP_PO_Creation
...
IB_06_TC06_Inbound_Happy_Path_WMS_Receiving
   ...
   [Teardown]   Run keywords   Log_Json_Object
    ...
    Comment  MOD - API - Purchase Order & OrderLine Validation - After PO Creation
       Wait Until Keyword Succeeds    ${Retry_IB01_WIT}      ${Retry_Interval_IB01_WIT}  API_PO_Validation  ${POSearch.Query}${ExcelData[1].V_PO_Number}[0]  ${POSearch.URL}  ${PurchaseOrder_Status.Initial}
       Wait Until Keyword Succeeds    ${Retry_IB01_WIT}      ${Retry_Interval_IB01_WIT}  API_POLine_Validation  ${POSearch.Query}${ExcelData[1].V_PO_Number}[0]   ${POLine.URL}    ${PO_Status_API}
    ...
    Comment  MOD - API - ASN Validation - After ASN Creation
        Wait Until Keyword Succeeds    ${Retry_IB01_WIT}    ${Retry_Interval_IB01_WIT}  API_ASN_Validation    ${ASNs_Status.Initial_API}
    ...
    Comment  MOD - API - Inbound Delivery Validation - Post Inbound Delivery Creation
        Wait Until Keyword Succeeds    ${Retry_IB01_WIT}      ${Retry_Interval_IB01_WIT}  API_INBOUND_DELIVERY_STATUS   ${IBDelivery_Status.Initial}
   ...
   Comment  MOD-UI- WM Mobile - Recv LPN - Initiate Transaction
      ...
      RECEIVE_LPN    IB01_WIT
   Comment  MOD - WM Mobile - Witron - Receiving
      Receive_DC_ASN_LPN_WITRON   IB01_WIT
   Comment  MOD - API - RTU message Validation
      Wait Until Keyword Succeeds    ${Retry_IB01_WIT}    ${Retry_Interval_IB01_WIT}   API_WITRON_RTU_VALIDATION
   Comment  MOD - WM Mobile- Witron - Receiving
      WITRON_INDUCTION_LPN_VENDOR_ASN  IB01_WIT
   Comment  MOD - API - RTU message Validation
      Wait Until Keyword Succeeds    ${Retry_IB01_WIT}    ${Retry_Interval_IB01_WIT}   API_WITRON_RTU_VALIDATION
   Comment  MOD - API -Witron - Post RTC message - Putaway
      RTC_WITRON_PUTAWAY
    Comment  MOD - API - Purchase Order & Orderline Validation -After Receiving
      Wait Until Keyword Succeeds    ${Retry_IB01_WIT}    ${Retry_Interval_IB01_WIT}   API_PO_Validation  ${POSearch.Query}${ExcelData[1].V_PO_Number}[0]  ${POSearch.URL}   ${PurchaseOrder_Status.AfterReceiving}
      Wait Until Keyword Succeeds    ${Retry_IB01_WIT}    ${Retry_Interval_IB01_WIT}  API_POLine_Validation  ${POSearch.Query}${ExcelData[1].V_PO_Number}[0]   ${POLine.URL}    ${PO_Status_API}
   Comment  MOD - API- ASN Validation - After Receiving
      Wait Until Keyword Succeeds    ${Retry_IB01_WIT}    ${Retry_Interval_IB01_WIT}  API_ASN_Validation    ${ASNs_Status.AfterReceiving}
   Comment  MOD - API- LPN Validation- After Receiving
      Wait Until Keyword Succeeds    ${Retry_IB01_WIT}    ${Retry_Interval_IB01_WIT}  API_LPN_Validation   WIT   IB   Consume_ILPN
   Comment  MOD - API - Inbound Delivery Validation - After Receiving
      Wait Until Keyword Succeeds    ${Retry_IB01_WIT}    ${Retry_Interval_IB01_WIT}  API_INBOUND_DELIVERY_STATUS   ${IBDelivery_Status.AfterReceiving}
   Comment  MOD - API- Pix Validation - After receiving
      Wait Until Keyword Succeeds    ${Retry_IB01_WIT_PIX}    ${Retry_Interval_IB01_WIT_PIX}  API_PIX_Transaction_Validation  ${PIXValidation.AfterReceiving}   MHE
   Comment  MOD - UI - Verify ASN
      VERIFY_ASN    ${ASN_ID}
    Comment  MOD - API- LPN Validation-After Verify ASN
      Wait Until Keyword Succeeds    ${Retry_IB01_WIT}    ${Retry_Interval_IB01_WIT}  API_LPN_Validation   WIT   IB   Consume_ILPN
   Comment   MOD - API-  Pix Validation - After Verify ASN
      Wait Until Keyword Succeeds    ${Retry_IB01_WIT_PIX}    ${Retry_Interval_IB01_WIT_PIX}      API_PIX_Transaction_Validation  ${PIXValidation.AfterVerifyASN}
   Comment   MOD - MIF-  Pix Validation - After Verify ASN
      Wait Until Keyword Succeeds    ${Retry_IB01_WIT}      ${Retry_Interval_IB01_WIT}  PIX_Data_Validation_Through_MIF_VERIFY_ASN
   Comment  MOD - API - MIF - Vendor ASN  - Good Receipt pix Validation
      Wait Until Keyword Succeeds    ${Retry_IB01_WIT}      ${Retry_Interval_IB01_WIT}  API_PIX_Validation_Verify_ASN_Good_receipt
   Comment  MOD - API - PIX - Vendor ASN  - Good Receipt pix Validation
      Wait Until Keyword Succeeds    ${Retry_IB01_WIT}      ${Retry_Interval_IB01_WIT}  PIX_Data_Validation_Through_MIF_Good_Receipt
   Comment  MOD - API- ASN Validation- After Verify ASN
      Wait Until Keyword Succeeds    ${Retry_IB01_WIT}    ${Retry_Interval_IB01_WIT}  API_ASN_Validation    ${ASNs_Status.AfterVerifyASN}   ${MHE_SSI.Vendor_ASN}   WIT
   Comment  MOD - API - Inbound Delivery Validation - After ASN Verification
      Wait Until Keyword Succeeds    ${Retry_IB01_WIT}    ${Retry_Interval_IB01_WIT}   API_INBOUND_DELIVERY_STATUS   ${IBDelivery_Status.AfterVerifyASN}    IB01_WIT
...
IB_01_TC_04_WMS_PO_CLose_Validation
    ...
    Comment  MOD - API - Purchase Order Validation - After PO Close
     Wait Until Keyword Succeeds    ${Retry_IB01_WIT}    ${Retry_Interval_IB01_WIT}   API_PO_Validation  ${POSearch.Query}${ExcelData[1].V_PO_Number}[0]  ${POSearch.URL}   ${PurchaseOrder_Status.Closed}
