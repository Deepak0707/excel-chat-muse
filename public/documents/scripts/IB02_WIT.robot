...
${Retry_IB02_WIT_PIX}   25x
${Retry_Interval_IB02_WIT_PIX}    30 seconds
...
IB02_WIT_TC01_Inbound_Happy_Path_WMS_Receiving
   ...
   [Teardown]   Run keywords   Log_Json_Object
    Comment  MOD - MIF - PO Creation MockUp
       ${PO_Creation_Json}   PO_CREATION_MOCKUP_DATA
    ...
    Comment  MOD - API - Purchase Order & OrderLine Validation - After PO Creation
       Wait Until Keyword Succeeds    ${Retry_IB02_WIT}      ${Retry_Interval_IB02_WIT}  API_PO_Validation  ${POSearch.Query}${ExcelData[1].V_PO_Number}[0]  ${POSearch.URL}  ${PurchaseOrder_Status.Initial}
       Wait Until Keyword Succeeds    ${Retry_IB02_WIT}      ${Retry_Interval_IB02_WIT}  API_POLine_Validation  ${POSearch.Query}${ExcelData[1].V_PO_Number}[0]   ${POLine.URL}    ${PO_Status_API}
    Comment  MOD - MIF - Vendor ASN creation through MIF
       ${Vendor_ASN_Creation}    Vendor_ASN_Creation_MockUp
    Comment  MOD - API - Purchase Order Validation - Post Vendor ASN Creation
       Wait Until Keyword Succeeds    ${Retry_IB02_WIT}      ${Retry_Interval_IB02_WIT}  API_PO_Validation  ${POSearch.Query}${ExcelData[1].V_PO_Number}[0]  ${POSearch.URL}  ${PurchaseOrder_Status.AfterASNAssign}
    Comment  MOD - API - ASN Validation - Post Vendor ASN Creation
       Wait Until Keyword Succeeds    ${Retry_IB02_WIT}      ${Retry_Interval_IB02_WIT}   API_ASN_Validation    ${ASNs_Status.Initial_API}
    Comment  MOD - API - Pre -Item Inventory Validation - Before Receiving
       ${Item_Inv_Pre}   API_Item_Inventory_Validation   Pre
    Comment  MOD - API - LPN Validation - Post Vendor ASN Creation
       Wait Until Keyword Succeeds    ${Retry_IB02_WIT}      ${Retry_Interval_IB02_WIT}   API_LPN_Validation   Vendor_ASN
   ...
   Comment  MOD - API - Inbound Delivery Validation - Post Inbound Delivery Creation
       Wait Until Keyword Succeeds    ${Retry_IB02_WIT}      ${Retry_Interval_IB02_WIT}  API_INBOUND_DELIVERY_STATUS   ${IBDelivery_Status.Initial}
   ...
   Comment  MOD -UI-Select Task Group and Vehicle Type
      TASKGROUP_VEHICLETYPE_SELECTION  ${TaskstobePerformed.Recv}
   Comment  MOD-UI- WM Mobile - Recv LPN - Initiate Transaction
      RECEIVE_LPN_INITIATE_TRANSACTION    ${DockDoorID}    ${Shipment_ID}
      RECEIVE_VENDOR_ASN_LPN    IB02_WIT
   Comment  MOD - WM Mobile - Witron - Receiving
      RECEIVE_VENDOR_ASN_LPN_WITRON   IB02_WIT
   Comment  MOD - API - RTU message Validation
      Wait Until Keyword Succeeds    ${Retry_IB02_WIT}    ${Retry_Interval_IB02_WIT}   API_WITRON_RTU_VALIDATION
   Comment  MOD - WM Mobile- Witron - Receiving
      WITRON_INDUCTION_LPN_VENDOR_ASN  IB02_WIT
   Comment  MOD - API - RTU message Validation
      Wait Until Keyword Succeeds    ${Retry_IB02_WIT}    ${Retry_Interval_IB02_WIT}   API_WITRON_RTU_VALIDATION
   Comment  MOD - API -Witron - Post RTC message - Putaway
      RTC_WITRON_PUTAWAY
    Comment  MOD - API - Purchase Order & Orderline Validation -After Receiving
      Wait Until Keyword Succeeds    ${Retry_IB02_WIT}    ${Retry_Interval_IB02_WIT}   API_PO_Validation  ${POSearch.Query}${ExcelData[1].V_PO_Number}[0]  ${POSearch.URL}   ${PurchaseOrder_Status.AfterReceiving}
      Wait Until Keyword Succeeds    ${Retry_IB02_WIT}    ${Retry_Interval_IB02_WIT}  API_POLine_Validation  ${POSearch.Query}${ExcelData[1].V_PO_Number}[0]   ${POLine.URL}    ${PO_Status_API}
   Comment  MOD - API- ASN Validation - After Receiving
      Wait Until Keyword Succeeds    ${Retry_IB02_WIT}    ${Retry_Interval_IB02_WIT}  API_ASN_Validation    ${ASNs_Status.AfterReceiving}
   Comment  MOD - API- LPN Validation- After Receiving
      Wait Until Keyword Succeeds    ${Retry_IB02_WIT}    ${Retry_Interval_IB02_WIT}  API_LPN_Validation   SSI   IB   Consume_LPN
   Comment  MOD - API - Inbound Delivery Validation - After Receiving
      Wait Until Keyword Succeeds    ${Retry_IB02_WIT}    ${Retry_Interval_IB02_WIT}  API_INBOUND_DELIVERY_STATUS   ${IBDelivery_Status.AfterReceiving}
   Comment  MOD - API- Pix Validation - After receiving
      Wait Until Keyword Succeeds    ${Retry_IB02_WIT_PIX}    ${Retry_Interval_IB02_WIT_PIX}  API_PIX_Transaction_Validation  ${PIXValidation.AfterReceiving}   MHE
   Comment  MOD - UI - Verify ASN
      VERIFY_ASN    ${ASN_ID}
   Comment  MOD - API- LPN Validation-After Verify ASN
      Wait Until Keyword Succeeds    ${Retry_IB02_WIT}    ${Retry_Interval_IB02_WIT}  API_LPN_Validation   SSI   IB   ${MHE_SSI.POST_GIC_Vendor_ASN}
   Comment   MOD - API-  Pix Validation - After Verify ASN
      Wait Until Keyword Succeeds    ${Retry_IB02_WIT_PIX}    ${Retry_Interval_IB02_WIT_PIX}     API_PIX_Validation_Verify_ASN_Vendor_ASN
   Comment   MOD - MIF-  Pix Validation - After Verify ASN
      Wait Until Keyword Succeeds    ${Retry_IB02_WIT}      ${Retry_Interval_IB02_WIT}  PIX_Data_Validation_Through_MIF_VENDOR_ASN
   Comment  MOD - API - MIF - Vendor ASN  - Good Receipt pix Validation
      Wait Until Keyword Succeeds    ${Retry_IB02_WIT}      ${Retry_Interval_IB02_WIT}  API_PIX_Validation_Verify_ASN_Good_receipt
   Comment  MOD - API - PIX - Vendor ASN  - Good Receipt pix Validation
      Wait Until Keyword Succeeds    ${Retry_IB02_WIT}      ${Retry_Interval_IB02_WIT}  PIX_Data_Validation_Through_MIF_Good_Receipt
   Comment  MOD - API- ASN Validation- After Verify ASN
      Wait Until Keyword Succeeds    ${Retry_IB02_WIT}    ${Retry_Interval_IB02_WIT}  API_ASN_Validation    ${ASNs_Status.AfterVerifyASN}   ${MHE_SSI.Vendor_ASN}   WIT
   ...
   Comment  MOD - API - Inbound Delivery Validation - After ASN Verification
      Wait Until Keyword Succeeds    ${Retry_IB02_WIT}    ${Retry_Interval_IB02_WIT}   API_INBOUND_DELIVERY_STATUS   ${IBDelivery_Status.AfterVerifyASN}
